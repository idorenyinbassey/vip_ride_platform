...
class PricingZone(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                check=models.Q(min_longitude__lt=models.F('max_longitude')),
    ...
    def clean(self):
        ...
        if self.min_latitude and self.max_latitude and self.min_latitude >= self.max_latitude:
            raise ValidationError({
                'min_latitude': 'Minimum latitude must be less than maximum latitude'
            })
        
        if self.min_longitude and self.max_longitude and self.min_longitude >= self.max_longitude:
            raise ValidationError({
                'min_longitude': 'Minimum longitude must be less than maximum longitude'
            })
...
class PromotionalCode(models.Model):
    ...
    def increment_usage_count(self):
        ...
        with transaction.atomic():
            PromotionalCode.objects.filter(id=self.id).update(
